...
import { deleteUserAccount, banUser, unbanUser } from '../store/index'; // Import the deleteUserAccount function
...
const CustomerDetail = ({navigation, route}) =>{
    ...
    const handleUpdate = async () => {
      try {  
        ...
        navigation.goBack(); // Thay đổi này
      } catch (error) {
        ...
        Alert.alert(
            ...
            [
                ...
                { 
                    ...
                    onPress: async () => {
                        try {
                            await unbanUser(email); // Call the unbanUser function
        ...
        Alert.alert(
            ...
            [
                ...
                { 
                    ...
                    onPress: async () => {
                        try {
                            // Đăng nhập lại để đảm bảo thông tin xác thực hợp lệ
                            const userCredential = await auth().signInWithEmailAndPassword(email, "user_password"); // Thay thế bằng mật khẩu thực tế
                            ...
                            console.log(`Tài khoản ${email} đã được xóa khỏi Firestore và Firebase Auth`);
                            Alert.alert("Thông báo", "Tài khoản đã được xóa thành công!");
                            navigation.navigate("Customers");
                        } catch (error) {
                            console.error("Error deleting account: ", error);
                            Alert.alert("Lỗi", "Có lỗi xảy ra khi xóa tài khoản. Vui lòng thử lại sau.");
                        }
                    },
                    style: "destructive"
                }
      ...
      <View style={styles.container}> 
          ...
          <View style={styles.buttonContainer}>
              ...
              <Button 
                ...
                onPress={() => navigation.goBack()} // Thay đổi này
              >
                  Trở về
              </Button>
              {!isBanned ? (
